...
const bottomNav = [
  {name:'GIS地图',path:'/gis',active:true},{name:'视频监控',path:'/monitor'},{name:'无人机',path:'/uav'},
  {name:'环境监测',path:'/env'},{name:'告警中心',path:'/alert'},{name:'数据统计',path:'/statistics'},
...
<template>
  <div class="h-screen w-screen bg-[#0A1628] flex flex-col overflow-hidden text-white" style="font-family:'PingFang SC','Microsoft YaHei','Helvetica Neue',sans-serif">

    <!-- TOP BAR -->
    <header class="top-bar">
      <div class="top-left" style="width:240px"><span style="font-size:14px;font-weight:600;color:var(--accent);font-family:var(--font-num)">{{ headerDate() }}</span></div>
      <div class="top-center"><div class="top-title">洽川湿地GIS指挥中心</div><div class="top-title-sub">智慧治理 · 一屏观全域</div></div>
      <div class="top-right"><span class="top-weather">☀️ 26°C · 晴</span><span class="top-time">{{ clockSec() }}</span><a href="/nav" class="top-exit">退出</a></div>
    </header>
    ...
    <main class="main">
      <div class="main-row">
        ...
        <aside class="left-panel">
          <div class="panel"><div class="panel-title">○ 合阳县 · 概况</div>
            <div style="text-align:center;padding:4px 0 8px;border-bottom:1px solid var(--border);margin-bottom:8px"><div style="font-size:28px;font-weight:700;color:var(--accent);font-family:var(--font-num)">1,342</div><div style="font-size:10px;color:var(--text3)">km² · 辖区面积</div></div>
            <div class="kpi-grid">
          ...
          <div class="panel"><div class="panel-title">● 今日核心指标</div>
            <div class="kpi-grid">
          ...
          <div class="panel" style="flex:1;min-height:0"><div class="panel-title">◎ 告警量 TOP5</div>
            <div class="rank-list"><div v-for="(r,i) in alertRank" :key="r.name" class="rank-item"><div class="rank-num" :class="{top:i<3}">{{ i+1 }}</div><span class="rank-name">{{ r.name }}</span><span class="rank-val">{{ r.val }}</span></div></div>
          </div>
        </aside>
        ...
        <section class="map-wrap">
          ...
          <div class="map-legend"><span class="legend-item"><span class="legend-dot" style="background:var(--accent)" />监控点位</span><span class="legend-item"><span class="legend-dot" style="background:var(--danger)" />告警区域</span><span class="legend-item"><span class="legend-dot" style="background:var(--green)" />正常区域</span></div>
          <div class="floating-alert-panel"><div class="floating-alert-title">⚠ 实时告警 <a href="/alert" style="color:var(--accent);font-size:9px;text-decoration:none;margin-left:auto">全部→</a></div><div class="floating-alert-item"><span class="event-dot red" /><span style="color:var(--text2);font-size:10px">核心区北门 非法闯入</span><span style="color:var(--text3);font-size:9px;font-family:var(--font-num);margin-left:auto">10:42</span></div><div class="floating-alert-item"><span class="event-dot yellow" /><span style="color:var(--text2);font-size:10px">CAM-0203 设备离线</span><span style="color:var(--text3);font-size:9px;font-family:var(--font-num);margin-left:auto">10:25</span></div><div class="floating-alert-item"><span class="event-dot yellow" /><span style="color:var(--text2);font-size:10px">水质监测站C区 异常</span><span style="color:var(--text3);font-size:9px;font-family:var(--font-num);margin-left:auto">10:18</span></div><div class="floating-alert-item"><span class="event-dot green" /><span style="color:var(--text2);font-size:10px">东岸火警误报 已处理</span><span style="color:var(--text3);font-size:9px;font-family:var(--font-num);margin-left:auto">09:45</span></div></div>
        </section>
        ...
        <aside class="right-panel">
          <div class="panel" style="flex:1;min-height:0;display:flex;flex-direction:column">
            ...
            <div class="chart-box" style="display:flex;flex-direction:column;gap:4px;padding:0 2px">
              <div v-for="(r,i) in alertRank" :key="r.name" class="rank-h-item">
                ...
                <div class="rank-h-track">
                  <div class="rank-h-bar" :style="{width:r.val/15*100+'%'}"></div>
                </div>
          ...
          <div class="panel" style="flex:1;min-height:0;display:flex;flex-direction:column">
            ...
            <div style="flex:1;display:flex;align-items:flex-end;gap:4px;padding:4px 2px 0">
              <div v-for="(b,i) in chartBars" :key="i" style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px">
          ...
          <div class="panel" style="flex:1;min-height:0;display:flex;flex-direction:column">
            ...
            <div style="flex:1;display:flex;align-items:center;gap:10px;padding:4px">
              <div style="width:70px;height:70px;border-radius:50%;background:conic-gradient(#00BFFF 0deg 135deg,#FFCC00 135deg 207deg,#FF3366 207deg 315deg,#00CC99 315deg 360deg);flex-shrink:0;border:2px solid var(--bg-deep)"></div>
              <div style="flex:1;display:flex;flex-direction:column;gap:3px;font-size:9px">
                <div style="display:flex;align-items:center;gap:4px"><span style="width:6px;height:6px;border-radius:50%;background:#00BFFF;flex-shrink:0"></span><span style="color:var(--text2)">非法闯入</span><span style="margin-left:auto;color:var(--accent);font-family:var(--font-num)">37.5%</span></div>
                <div style="display:flex;align-items:center;gap:4px"><span style="width:6px;height:6px;border-radius:50%;background:#FFCC00;flex-shrink:0"></span><span style="color:var(--text2)">设备故障</span><span style="margin-left:auto;color:var(--accent);font-family:var(--font-num)">20.0%</span></div>
                <div style="display:flex;align-items:center;gap:4px"><span style="width:6px;height:6px;border-radius:50%;background:#FF3366;flex-shrink:0"></span><span style="color:var(--text2)">环境异常</span><span style="margin-left:auto;color:var(--accent);font-family:var(--font-num)">30.0%</span></div>
                <div style="display:flex;align-items:center;gap:4px"><span style="width:6px;height:6px;border-radius:50%;background:#00CC99;flex-shrink:0"></span><span style="color:var(--text2)">消防预警</span><span style="margin-left:auto;color:var(--accent);font-family:var(--font-num)">12.5%</span></div>
              </div>
            </div>
          </div>
        </aside>
      ...
      <nav class="bottom-nav">
        <button v-for="nav in bottomNav" :key="nav.name" class="nav-btn" :class="{active:nav.active}" @click="router.push(nav.path)"><span class="nav-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="10" r="3"/><path d="M12 2C7 2 3 6.5 3 12c0 2 .5 3.5 1.5 5L12 22l7.5-5c1-1.5 1.5-3 1.5-5 0-5.5-4-10-9-10z"/></svg></span><span>{{ nav.name }}</span></button>
      </nav>
    </main>
  </div>
</template>
...
.rank-h-name { width:56px;color:var(--text2);flex-shrink:0;overflow:hidden; }
...
.nav-btn { position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;padding:4px 10px;background:transparent;border:1px solid transparent;border-radius:6px;color:var(--text3);font-size:10px;cursor:pointer;transition:all .25s;min-width:56px; }
...
.nav-icon svg { width:16px;height:16px;display:block;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round; }
